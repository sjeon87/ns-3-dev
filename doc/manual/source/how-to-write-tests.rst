...
If the test output that you are confirming is sensitive to perturbations from different
random variable stream assignments (which may cause previously passing test macros to fail),
it is recommended to lock down the specific stream in use in your test program.

There are two means by which the stream (a pseudorandom sequence of numbers) provided to
a random variable can be changed:

1) Seed or run number assignment:   Changing RngSeed or RngRun will change all of the
   pseudorandom sequences in the simulation.
2) Stream assignment:  For the (very large) pseudorandom stream of numbers generated by a given
   RngSeed and RngRun, each random variable is mapped to a specific subset of the bigger stream.
   By default, random variables are lazily assigned to streams (this is called automatic
   stream assignment) but they can be assigned to specific stream sequences through the use of
   ``AssignStream()`` helper methods and ``SetStream()`` directly on the random variable itself.

We recommend to always keep RngSeed at the default value of 1, and vary the RngRun number to
obtain different streams (see the chapter on random variables for why this is so) but the API
exists to change the RngSeed so we will guard against its changes, just in case.

Sometimes, people will want to expose tests to different RngRun numbers, so there is no guarantee
that it will be set to the default value of 1.  Furthermore, because of the way tests are
combined into one executable, if your test assigns the global RngRun variable within the scope
of its test execution, that assignment will propagate to other tests if not undone at the
end of the test.  The following code, taken from ``wifi-mac-ofdma-test.cc``, demonstrates
how to cache the existing values of RngSeed and RngRun, change them for the scope of a test
execution, and restore them to the cached values at the end of the test.::
...
In addition to the above, users should call ``AssignStreams`` on all simulation helpers and
check for unassigned streams to be certain that the test configuration is locked to specific
instances of random variable streams (if desired).
