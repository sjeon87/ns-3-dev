...
Automatic streams may introduce unexpected randomness
*****************************************************

Automatic streams, as the name implies, are assigned to random stream
variables at the time of their instantiation. However, it can be easy
to inadvertently come to rely on a specific automatically assigned
stream number, which may lead to unexpected failures, for example during testing.

This happens because any new random stream created before the one you depend
on will shift the numbering of all subsequent random variables. Such shifts
can occur, for instance, when you build ns-3 with a different set of modules,
or when updated models in ns-3 introduce additional random streams.

To obtain fully reproducible results, it is essential to assign streams manually
and ensure that their mappings remain consistent across ns-3 versions.
This can be challenging, since classes may have deeply nested relationships,
and it is easy to miss assigning one.

For this reason, the ns-3 core includes an instrumented option that traces
the origin of automatically assigned streams. To use it, run your program
with the environment variable ``NS_CHECK_AUTOMATIC_STREAMS`` set. This will
print a list of offending random stream types along with their stack traces.
If your program does not call Simulator::Run(), for example in a unit test,
manually call ``RandomVariableStream::CheckAutomaticStreams()`` after
finishing the simulation setup.

This check can also be used to identify which examples or tests fully set
all used streams, by simply running ``NS_CHECK_AUTOMATIC_STREAMS=1 ./test.py``.

One example of the output generated by adding the environment flag is shown below:

.. sourcecode:: console

  $ NS_CHECK_AUTOMATIC_STREAMS=1 ./ns3 run "test-runner --test-name=outdoor-random-walk-model"
  ...
  Automatic variable stream: ns3::UniformRandomVariable, instantiated in:
     0# ns3::RandomVariableStream::SetStream(long) at :0
     1# ns3::AccessorHelper<ns3::RandomVariableStream, ns3::IntegerValue>::Set(ns3::ObjectBase*, ns3::AttributeValue const&) const at :0
     2# ns3::ObjectBase::DoSet(ns3::Ptr<ns3::AttributeAccessor const>, ns3::Ptr<ns3::AttributeChecker const>, ns3::AttributeValue const&) [clone .localalias] at :0
     3# ns3::ObjectBase::ConstructSelf(ns3::AttributeConstructionList const&) at :0
     4# ns3::ObjectFactory::Create() const at :0
     5# ns3::Ptr<ns3::Object> ns3::ObjectFactory::Create<ns3::Object>() const at :0
     6# ns3::PointerValue::DeserializeFromString(std::__cxx11::basic_string<char, std::char_traits<char>, std::allocator<char> >, ns3::Ptr<ns3::AttributeChecker const>) at :0
     7# ns3::AttributeChecker::CreateValidValue(ns3::AttributeValue const&) const at :0
     8# ns3::ObjectBase::DoSet(ns3::Ptr<ns3::AttributeAccessor const>, ns3::Ptr<ns3::AttributeChecker const>, ns3::AttributeValue const&) [clone .localalias] at :0
     9# ns3::ObjectBase::ConstructSelf(ns3::AttributeConstructionList const&) at :0
    10# OutdoorRandomWalkTestCase::DoRun() at :0
    11# ns3::TestCase::Run(ns3::TestRunnerImpl*) at :0
    12# ns3::TestCase::Run(ns3::TestRunnerImpl*) at :0
    13# ns3::TestRunnerImpl::Run(int, char**) at :0
    14# ns3::TestRunner::Run(int, char**) at :0
    15# __libc_start_call_main at ../sysdeps/nptl/libc_start_call_main.h:58
    16# __libc_start_main_impl at ../csu/libc-start.c:360
    17# _start at :0
    18#

  aborted. msg="NS_CHECK_AUTOMATIC_STREAMS: 9 automatic variable streams", +10001.000000000s -1 file=/mnt/c/tools/sources/ns-3-dev/src/core/model/random-variable-stream.cc, line=195
  NS_FATAL, terminating
     0# ns3::RandomVariableStream::CheckAutomaticStreams() at :0
     1# ns3::Simulator::Stop() at :0
     2# ns3::DefaultSimulatorImpl::ProcessOneEvent() at :0
     3# ns3::DefaultSimulatorImpl::Run() [clone .localalias] at :0
     4# OutdoorRandomWalkTestCase::DoRun() at :0
     5# ns3::TestCase::Run(ns3::TestRunnerImpl*) at :0
     6# ns3::TestCase::Run(ns3::TestRunnerImpl*) at :0
     7# ns3::TestRunnerImpl::Run(int, char**) at :0
     8# ns3::TestRunner::Run(int, char**) at :0
     9# __libc_start_call_main at ../sysdeps/nptl/libc_start_call_main.h:58
    10# __libc_start_main_impl at ../csu/libc-start.c:360
    11# _start at :0
    12#

  terminate called without an active exception
  Command 'build/utils/ns3-dev-test-runner --test-name=outdoor-random-walk-model' died with <Signals.SIGABRT: 6>.

From that we can see that 9 automatically assigned streams exist for this program (``test-runner``).
The last one shown was created during the static initialization of a ``UniformRandomVariable``,
created by the ``RateErrorModel`` inside of ``ThreeGppHttpObjectTestCase``.

We can then start to fix this issues, by locating the objects that hold these specific uninitialized streams.
This becomes easier after rebuilding in debugging mode, where lines of function calls will be printed.
After locating them, we need to make sure the streams are set. This is typically done via a custom ``AssignStreams()``
function, that assigns all random streams of a given object and recursively calls ``AssignStreams``
of objects it has pointers too.
