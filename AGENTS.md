...
- **Logging**: Conventions for including log statements and for choosing log severity levels (the meanings of `LOG_INFO`, `LOG_LOGIC`, `LOG_DEBUG`, etc.) and for using `NS_LOG_FUNCTION`, including logging in constructors and destructors, are in `doc/manual/source/logging-asserts.rst` (Guidelines section).
