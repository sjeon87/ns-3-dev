...
/**
 * Descriptor for AttributeValue documentation generated by helper macros.
 */
struct AttributeDocumentationRegistration
{
    std::string m_name;   //!< The base name of the AttributeValue type.
    std::string m_header; //!< The file where the AttributeValue helper was declared.
};

/**
 * Register an AttributeValue helper type for documentation validation.
 *
 * Helper macros call this during static initialization so that tools such as
 * print-introspected-doxygen can validate that their manual documentation
 * tables are complete.
 *
 * @param [in] name The base name of the AttributeValue type.
 * @param [in] header The file where the helper was declared.
 * @returns Always true.
 */
CORE_EXPORT bool RegisterAttributeDocumentation(const std::string& name, const std::string& header);

/**
 * Get the list of AttributeValue helper types registered for documentation.
 *
 * @returns The registrations collected during static initialization.
 */
CORE_EXPORT const std::vector<AttributeDocumentationRegistration>&
