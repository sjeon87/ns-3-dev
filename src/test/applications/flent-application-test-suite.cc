 ...
 */
 ...
 /*
  This test suite:
    - Runs the first 4 tests for sanity check and generating the flent files required for the Flent file integrity and Flent results test.
      Flent files generated from the first 4 tests can be reused again for the Flent file integrity and Flent results test.
    - Runs the Flent file integrity test which reads the files from first 4 tests and check if metadata required for flent-gui is available
    - Runs the Flent result test which reads the files from the 4 tests and calculate average throughput and average ICMP latency.
...
{
  std::vector<std::string> flentFiles {"rrul.flent", "tcp_upload.flent", "tcp_download.flent", "ping.flent"};
  for (unsigned int i = 0; i < flentFiles.size (); i++)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ ((itr_name++).key ().asString (), "metadata", "in file " + flentFiles[i] + " metadata not available");
      NS_TEST_ASSERT_MSG_EQ ((itr_name++).key ().asString (), "raw_values", "in file " + flentFiles[i] + " raw_values not available");
      NS_TEST_ASSERT_MSG_EQ ((itr_name++).key ().asString (), "results", "in file " + flentFiles[i] + " results not available");
      NS_TEST_ASSERT_MSG_EQ ((itr_name++).key ().asString (), "version", "in file " + flentFiles[i] + " version not available");
      NS_TEST_ASSERT_MSG_EQ ((itr_name).key ().asString (), "x_values", "in file " + flentFiles[i] + " x_values not available");

      for (auto itrRawValues = root["raw_values"].begin (), itrResults = root["results"].begin (); itrRawValues != root["raw_values"].end (); itrRawValues++, itrResults++)
        {
          NS_TEST_ASSERT_MSG_EQ (itrRawValues.key ().asString (), itrResults.key ().asString (), flentFiles[i] + " number of results and raw values do not match");
          NS_TEST_ASSERT_MSG_EQ (root["results"][itrResults.key ().asString ()].size(), root["x_values"].size (), flentFiles[i] + " number of results and raw values does not match");
        }
    }
  return;
}
 ...
 * Bounds are kept as per the results obtained from running the test and calculating the average throughput and average ICMP latency.
 */
  ...
  for (unsigned int i = 0; i < flentFiles.size (); i++)
    {
      ...
      if (flentFiles[i] == "tcp_upload.flent")
        {
          for (auto itrTcpUpload = root["results"]["TCP upload"].begin (); itrTcpUpload != root["results"]["TCP upload"].end (); itrTcpUpload++)
            {
          ...
          NS_TEST_ASSERT_MSG_LT (throughput/count, 50, flentFiles[i] + " TCP upload throughput should be less than 50");
          NS_TEST_ASSERT_MSG_GT (throughput/count, 43, flentFiles[i] + " TCP upload throughput should be greater than 43");

          count = 0;
          for (auto itrPing = root["results"]["Ping (ms) ICMP"].begin (); itrPing != root["results"]["Ping (ms) ICMP"].end (); itrPing++)
            {
          ...
          NS_TEST_ASSERT_MSG_LT (pingLatency/count, 82, flentFiles[i] + " Ping latency should be less than 82");
          NS_TEST_ASSERT_MSG_GT (pingLatency/count, 80, flentFiles[i] + " Ping latency throughput should be greater than 80");
        }
      else if (flentFiles[i] == "tcp_download.flent")
        {
          for (auto itrTcpDownload = root["results"]["TCP download"].begin (); itrTcpDownload != root["results"]["TCP download"].end (); itrTcpDownload++)
            {
          ...
          NS_TEST_ASSERT_MSG_LT (throughput/count, 50, flentFiles[i] + " TCP Download throughput should be less than 50");
          NS_TEST_ASSERT_MSG_GT (throughput/count, 43, flentFiles[i] + " TCP Download throughput should be greater than 43");
        ...
        {
          for (auto itrTcpDownload = root["results"]["TCP download BE"].begin (); itrTcpDownload != root["results"]["TCP download BE"].end (); itrTcpDownload++)
            {
              if ((*itrTcpDownload).asString () == "0.0")
                {
                  continue;
          ...
          NS_TEST_ASSERT_MSG_LT (throughput/count, 12, flentFiles[i] + " TCP Download throughput should be less than 12");
          NS_TEST_ASSERT_MSG_GT (throughput/count, 11, flentFiles[i] + " TCP Download throughput should be greater than 11");

          count = 0;
          throughput = 0.0;
          for (auto itrTcpDownload = root["results"]["TCP download BK"].begin (); itrTcpDownload != root["results"]["TCP download BK"].end (); itrTcpDownload++)
            {
              if ((*itrTcpDownload).asString() == "0.0")
                {
                  continue;
          ...
          NS_TEST_ASSERT_MSG_LT (pingLatency/count, 84, flentFiles[i] + " Ping latency should be less than 84");
          NS_TEST_ASSERT_MSG_GT (pingLatency/count, 80, flentFiles[i] + " Ping latency throughput should be greater than 80");
        }
    }
  return;
...
static FlentApplicationTestSuite g_FlentApplicationTestSuite; //!< Static variable for test initialization
