...
LrWpanSlottedCsmacaTestCase::DoRun()
{
    ...
    symbolRate = dev1->GetMac()->GetPhy()->GetDataOrSymbolRate(false);
    activePeriodsSum = m_apBoundary - m_startCap;
    boundary = std::fmod((activePeriodsSum * symbolRate).GetSeconds(), activePeriodSize);
    ...
    transactionTime = Seconds((m_transCost - (ifsSize + 12)) / symbolRate);
