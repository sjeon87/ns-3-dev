...
LrWpanCsmaCa::GetTimeToNextSlot() const
{
    ...
    Time nextBoundary;
    double elapsedSuperframeSymbols;
    double symbolRate = m_mac->GetPhy()->GetDataOrSymbolRate(false); // symbols per second
    Time timeAtBoundary;
    ...
    if (m_coorDest)
    {
        ...
        Time beaconTime [[maybe_unused]] = Seconds(m_mac->m_rxBeaconSymbols / symbolRate);
    ...
    symbolsToBoundary = lrwpan::aUnitBackoffPeriod -
                        std::fmod(elapsedSuperframeSymbols, lrwpan::aUnitBackoffPeriod);

    timeAtBoundary = Seconds((elapsedSuperframeSymbols + symbolsToBoundary) / symbolRate);
...
LrWpanCsmaCa::RandomBackoffDelay()
{
    ...
    uint64_t upperBound = (1ULL << m_BE) - 1;
    Time randomBackoff;
    double symbolRate;
    Time timeLeftInCap;

    symbolRate = m_mac->GetPhy()->GetDataOrSymbolRate(false); // symbols per second
    ...
    randomBackoff = Seconds((m_randomBackoffPeriodsLeft * lrwpan::aUnitBackoffPeriod) / symbolRate);
...
LrWpanCsmaCa::GetTimeLeftInCap()
{
    ...
    { // Take Incoming frame reference
        ...
        endCapTime = m_mac->m_macBeaconRxTime + Seconds(capSymbols / symbolRate);
    }
    else
    { // Take Outgoing frame reference
        activeSlot = m_mac->m_superframeDuration / 16;
        capSymbols = activeSlot * (m_mac->m_fnlCapSlot + 1);
        endCapTime = m_mac->m_macBeaconTxTime + Seconds(capSymbols / symbolRate);
    }
...
LrWpanCsmaCa::CanProceed()
{
    ...
    ccaSymbols = 0;
    m_randomBackoffPeriodsLeft = 0;
    symbolRate = m_mac->GetPhy()->GetDataOrSymbolRate(false);
    timeLeftInCap = GetTimeLeftInCap();
    ...
    transactionTime = Seconds(transactionSymbols / symbolRate);
