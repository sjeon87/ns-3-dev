...
class ClientServiceStatus : public Object
{
  public:
     ...
     * @param data Data to deliver to the client service instance. (used by RED_PART_RCV &
     * GP_SEGMENT_RCV)
...
class LtpProtocol : public Object
{
  public:
    ...
    /* \brief Must be called by clients to register their uint64_t, this
  ...
  private:
     ...
     */
    void EncapsulateBlockData(uint64_t dstClientService,
     ...
     */
    void ReportSegmentAckTransmission(SessionId id,
    ...
    ConvergenceLayerAdapters
        m_clas; //!< Mapping LtpEngineId with corresponding point-to-point link.
    ...
    Time m_onewayLightTime; //!< On way light time: time required to reach the remote ltp engine
                            //!< (for use in timers)
    ...
    std::queue<ActivationInterval> m_localOperatingSchedule; //!< Time intervals that the local LTP
                                                             //!< engine expects to be operating
};
