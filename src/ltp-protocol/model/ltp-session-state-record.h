...
class SessionStateRecord : public Object
{
  ...
  protected:
    ...
    /* Checkpoints*/
    uint64_t m_firstCpSerialNumber;   //!< First checkpoint serial number chosen(sender)/received
                                      //!< (received)
    uint64_t m_currentCpSerialNumber; //!< Next checkpoint serial number (sender) / Last checkpoint
                                      //!< number received (receiver)

    /* Reception Reports */
    uint64_t
        m_firstRpSerialNumber; //!< First report serial number chosen(receiver)/received (sender)
    uint64_t m_currentRpSerialNumber; //!< Next report serial number (receiver) / Last report serial
                                      //!< number received (sender)
