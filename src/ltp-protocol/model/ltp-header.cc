...
LtpHeader::Print(std::ostream& os) const
{
    ...
    os << "LtpHeader ( Version: " << (uint32_t)m_version << ", Type : " << (uint32_t)m_typeFlags
       << ", (SessionID ( Originator : " << m_sessionId.GetSessionOriginator()
       << ", Number: " << m_sessionId.GetSessionNumber()
       << "), HeaderExtensions: " << (uint32_t)m_hdrExtensionCnt
...
LtpHeader::GetSerializedSize(void) const
{
    ...
    Sdnv codec;
    size +=
        codec.Encode(m_sessionId.GetSessionOriginator()).size(); // SDNV encoded Session Originator
...
LtpContentHeader::GetSerializedSize(void) const
{
    ...
    /*  Data Segment */
    if ((m_type == LTPTYPE_RD) || (m_type == LTPTYPE_RD_CP) || (m_type == LTPTYPE_RD_CP_EORP) ||
        (m_type == LTPTYPE_RD_CP_EORP_EOB) || (m_type == LTPTYPE_GD) ||
        (m_type == LTPTYPE_GD_UF1) || (m_type == LTPTYPE_GD_UF2) || (m_type == LTPTYPE_GD_EOB))
    {
        size += codec.Encode(m_clientServiceId).size();
        size += codec.Encode(m_offset).size();
        size += codec.Encode(m_length).size();

        if ((m_type == LTPTYPE_RD_CP) || (m_type == LTPTYPE_RD_CP_EORP) ||
...
LtpContentHeader::Print(std::ostream& os) const
{
    ...
    if ((m_type == LTPTYPE_RD) || (m_type == LTPTYPE_RD_CP) || (m_type == LTPTYPE_RD_CP_EORP) ||
        ...
        (m_type == LTPTYPE_GD_UF1) || (m_type == LTPTYPE_GD_UF2) || (m_type == LTPTYPE_GD_EOB))
    {
        os << " Dst. Client Service ID: " << m_clientServiceId << " Offset: " << m_offset
           << ", Length: " << m_length;

        if ((m_type == LTPTYPE_RD_CP) || (m_type == LTPTYPE_RD_CP_EORP) ||
            (m_type == LTPTYPE_RD_CP_EORP_EOB))
        {
            os << ", CP Serial Num: " << m_cpSerialNumber
               << ", RP Serial Num: " << m_rpSerialNumber;
        }
    }
    else if (m_type == LTPTYPE_RS) /* Report Segments*/
    {
        os << "CP Serial Num: " << m_cpSerialNumber << ", RP Serial Num: " << m_rpSerialNumber
           << ", Lower Bound: " << m_lowerBound << ", Upper Bound: " << m_upperBound
           << ", RxClaim Count : " << m_rxClaimCnt;
    }
    else if (m_type == LTPTYPE_RAS) /* Report ACK Segment*/
    {
...
LtpContentHeader::SetCpSerialNumber(uint64_t num)
{
    ...
    if ((m_type == LTPTYPE_RD_CP) || (m_type == LTPTYPE_RD_CP_EORP) ||
        (m_type == LTPTYPE_RD_CP_EORP_EOB) || (m_type == LTPTYPE_RS) || (m_type == LTPTYPE_RAS))
    {
...
LtpContentHeader::AddReceptionClaim(ReceptionClaim claim)
{
    ...
    if (m_type == LTPTYPE_RS)
    {
        ...
        valid =
            ((claim.length > 1) &&
             (claim.length <= max_len) /* Reception Claim Length regulations */
             && ((claim.offset >= min_offset) ||
                 (claim.offset == 0 && min_offset == 0)) /* Reception Claim Offset regulations */
             && (claim_upperBound <=
                 m_upperBound) /* Claim upper bound must not exceed the report upper bound*/
            );
        ...
        {
            NS_FATAL_ERROR("Wrong Claim Boundaries - Offset:"
                           << claim.offset << " Length:" << claim.length << " "
                           << ((claim.length > 1) && (claim.length <= max_len)) << " "
                           << m_upperBound);
        }
...
bool
LtpExtension::operator==(const LtpExtension& o) const
{
    return ((m_extType == o.m_extType) && (m_len == o.m_len) && (m_value == o.m_value));
}

bool
LtpHeader::operator==(const LtpHeader& o) const
{
    return ((m_version == o.m_version) && (m_typeFlags == o.m_typeFlags) &&
            (m_sessionId == o.m_sessionId) && (m_hdrExtensionCnt == o.m_hdrExtensionCnt) &&
            (m_trailerExtensionCnt == o.m_trailerExtensionCnt) && (m_extensions == o.m_extensions));
}

bool
LtpTrailer::operator==(const LtpTrailer& o) const
{
...
LtpContentHeader::operator==(const LtpContentHeader& o) const
{
    ...
    if (m_type == o.m_type)
    {
        if ((m_type == LTPTYPE_RD) || (m_type == LTPTYPE_RD_CP) || (m_type == LTPTYPE_RD_CP_EORP) ||
            ...
            (m_type == LTPTYPE_GD_UF1) || (m_type == LTPTYPE_GD_UF2) || (m_type == LTPTYPE_GD_EOB))
        {
            result = ((m_clientServiceId == o.m_clientServiceId) && (m_offset == o.m_offset) &&
                      (m_length == o.m_length));

            if ((m_type == LTPTYPE_RD_CP) || (m_type == LTPTYPE_RD_CP_EORP) ||
                (m_type == LTPTYPE_RD_CP_EORP_EOB))
            {
                result = (result && (m_cpSerialNumber == o.m_cpSerialNumber) &&
        ...
        else if (m_type == LTPTYPE_RS) /* Report Segments*/
        {
            result = ((m_rpSerialNumber == o.m_rpSerialNumber) &&
                      ...
                      (m_upperBound == o.m_upperBound) && (m_lowerBound == o.m_lowerBound) &&
                      (m_rxClaimCnt == o.m_rxClaimCnt) && (m_rxClaims == o.m_rxClaims));
        }
        else if (m_type == LTPTYPE_RAS) /* Report ACK Segment*/
        {
            result = (m_rpSerialNumber == o.m_rpSerialNumber);
...
bool
LtpContentHeader::ReceptionClaim::operator<(const ReceptionClaim& o) const
{
