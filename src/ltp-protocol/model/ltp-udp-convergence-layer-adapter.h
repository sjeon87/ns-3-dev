...
class LtpUdpConvergenceLayerAdapter : public LtpConvergenceLayerAdapter
{
  public:
     ...
     *
     * @return 0 if operation failed (e.g. binding is not there); otherwise, return the MTU in
     * bytes.   *
     */
  ...
  private:
    ...
    std::map<uint64_t, Ptr<Socket>>
        m_l4SendSockets; //!< Socket for sending data to remote LTP egnines.
