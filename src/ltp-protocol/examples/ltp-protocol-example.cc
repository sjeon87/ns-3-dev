  ...
  if (code == ns3::ltp::GP_SEGMENT_RCV)
    {
      ...
      std::cout << "ClientServiceNotification - Received a Green Data Segment of Size: ( " << dataLength << ")" << std::endl;
    }
  ...
  if (code == ns3::ltp::RED_PART_RCV)
    {
      ...
      std::cout << "ClientServiceNotification - Received Full Red Part of Size: ( " << dataLength << ")" << std::endl;

    }
  if (code == ns3::ltp::SESSION_END)
    {
      std::cout << "ClientServiceNotification - Received a full block of data with size: ( " << totalData << ") from LtpEngine: " << srcLtpEngine << std::endl;
    }
