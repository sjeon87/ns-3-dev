...
LtpSessionStateRecordTestCase::DoRun (void)
{
  ...
  Ptr<SenderSessionStateRecord> ssend = CreateObject<SenderSessionStateRecord> (srcEngine, srcClient, destEngine,destClient, number);
  ...
  Ptr<ReceiverSessionStateRecord> srecv = CreateObject<ReceiverSessionStateRecord> (srcEngine,srcClient,id, number);
