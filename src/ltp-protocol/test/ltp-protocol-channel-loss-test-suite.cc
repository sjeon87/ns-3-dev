...
class LtpProtocolRetransTestCase : public TestCase
{
  public:
    ...
    LtpProtocolRetransTestCase(uint32_t sentData,
                               uint32_t expectedData,
                               uint32_t redPartSz,
                               std::list<uint32_t> listReceiver,
                               std::list<uint32_t> listSender);
  ...
  private:
    ...
    std::list<uint32_t>
        m_lossesReceiver; // List of packets that will be lost upon reception at the receiver
    std::list<uint32_t>
        m_lossesSender; // List of packets that will be lost upon reception at the sender
};
...
LtpProtocolRetransTestCase::LtpProtocolRetransTestCase(uint32_t sentData,
                                                       uint32_t expectedData,
                                                       uint32_t redPartSz,
                                                       std::list<uint32_t> listReceiver,
                                                       std::list<uint32_t> listSender)
                                                                  ...
                                                                  uint32_t offset)
{
    ...
    std::cout << "Code :" << code << " " << dataLength << " " << m_rcvData << " " << endFlag
              << std::endl;
...
LtpProtocolRetransTestCase::DoRun(void)
{
    ...
    CallbackBase cb =
        MakeCallback(&LtpProtocolRetransTestCase::ClientServiceInstanceNotificationsSnd, this);
    CallbackBase cb2 =
        MakeCallback(&LtpProtocolRetransTestCase::ClientServiceInstanceNotificationsRcv, this);
...
LtpProtocolChannelLossTestSuite::LtpProtocolChannelLossTestSuite()
    : TestSuite("ltp-channel-loss", Type::UNIT)
{
    ...
    // Test 1: No losses
    AddTestCase(new LtpProtocolRetransTestCase(block_len,
                                               expected,
                                               red_len,
                                               receiver_losses,
                                               sender_losses),
    ...
    /* CCDS 743.1-R2: 3.4.2 Notes: Green-part data is not reliably transmitted under LTP. In
       particular, if there is green- part data in a block, the LTP segment containing the EOB
       marker may not be delivered to the destination. As a consequence, if the session contains
       green-part data there is no way for an LTP sender to know when an LTP receiver has closed a
            session.*/

    // Test 14: A Green EOB Segment is lost -- Session in receiver side is not closed, remains in a
    // waiting for EOB state.
    receiver_losses.clear();
