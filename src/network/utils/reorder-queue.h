 ...
 * @ingroup queue
 *
 * @brief A ReorderQueue introduces controlled packet reordering. After a fixed number of in-order
 * dequeues, it holds one packet and allows subsequent packets to bypass it before releasing the
 * held packet.
 */
...
class ReorderQueue : public Queue<Item>
{
  ...
  private:
    ...
    uint32_t m_bytesInQueue{0};      //!< actual bytes in the queue
    ...
    uint32_t m_holdCount{0};         //!< number of packets currently held for reordering
    uint32_t m_inSequenceCount{0};   //!< number of in-order packets delivered
    Ptr<Packet> m_held{nullptr};     //!< pointer to the currently held packet/item
