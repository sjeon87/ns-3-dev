...
UnboundedSkewClock::UnboundedSkewClock(double u_minSkew, double u_maxSkew,
                                       uint32_t u_numSkews)
    : m_ptime(Simulator::Now()),
      m_lastreadptime(Simulator::Now()),
      m_index(0)
{
  ...
  // Note: m_skewVariable must be instantiated before this runs.
  // If created via CreateObject, Attributes populate automatically.
  // If created via new, m_skewVariable might be null here.
  if (!m_skewVariable)
    {
      m_skewVariable = CreateObject<UniformRandomVariable>();
    }
  ...
  NS_LOG_INFO("Initialized skew values between " << u_minSkew << " and "
                                                 << u_maxSkew);
}
