...
template <typename Item>
ReorderQueue<Item>::~ReorderQueue() = default;
...
ReorderQueue<Item>::Dequeue()
{
    ...
    else
    {
        if (m_inSequenceCount == m_inSequenceLength)
        {
            ...
            if (m_packets.empty())
            {
                // Release the held packet because no other packets remain to reorder with.
                NS_LOG_LOGIC("Release " << m_held->GetUid()
                                        << " because no other packets remain to reorder with");
                p = m_held;
                m_held = nullptr;
                m_bytesInQueue -= p->GetSize();
                return p;
