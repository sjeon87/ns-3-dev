...
/**
 * Map of expected PER values, indexed by MCS, size (in bytes) and SNR (in dB), used to verify
 * the PER calculated by the TableBasedErrorRateModel.
 *
 * These values are snapshots of the model's own output, not the MATLAB link-simulation results
 * stored in error-rate-tables.cc. Each entry records 1 - GetChunkSuccessRate(mode, txVector,
 * snr, size * 8) for a default-constructed model and a WifiTxVector configured only with the
 * mode under test (hence 20 MHz, single user), with snr passed as a linear ratio and swept
 * from -4 dB to 30 dB in steps of 0.25 dB.
 */
