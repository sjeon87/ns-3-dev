...
WifiAssociationTestBase::WifiAssociationTestBase(const std::string& name,
    ...
    : TestCase(name + ", " + (std::ostringstream{} << baseParams).str()),
...
WifiAssociationTestBase::DoSetup()
{
    ...
    m_ap1Mac->TraceConnectWithoutContext(
        "AssociatedSta",
        MakeCallback(&WifiAssociationTestBase::AssocSeenByAp, this).Bind(PeekPointer(m_ap1Mac)));
    m_ap1Mac->TraceConnectWithoutContext(
        "DeAssociatedSta",
        MakeCallback(&WifiAssociationTestBase::DisassocSeenByAp, this).Bind(PeekPointer(m_ap1Mac)));

    m_ap2Mac->TraceConnectWithoutContext(
        "AssociatedSta",
        MakeCallback(&WifiAssociationTestBase::AssocSeenByAp, this).Bind(PeekPointer(m_ap2Mac)));
    m_ap2Mac->TraceConnectWithoutContext(
        "DeAssociatedSta",
        MakeCallback(&WifiAssociationTestBase::DisassocSeenByAp, this).Bind(PeekPointer(m_ap2Mac)));
}
...
WifiDisassocAndReassocTest::WifiDisassocAndReassocTest(const BaseParams& baseParams,
                                                       const Params& params)
    : WifiAssociationTestBase("Disassociation and reassociation test, " +
                                  (std::ostringstream{} << params).str(),
                              baseParams),
      m_params(params),
...
void
WifiDisassocAndReassocTest::AssocSeenByAp(ApWifiMac* apMac, uint16_t aid, Mac48Address staAddress)
{
...
void
WifiDisassocAndReassocTest::DisassocSeenByAp(ApWifiMac* apMac,
                                             uint16_t aid,
                                             Mac48Address staAddress)
{
    const auto addrByAid = apMac->GetMldOrLinkAddressByAid(aid);

    NS_TEST_ASSERT_MSG_EQ(addrByAid.has_value(),
                          true,
                          "AID of disassociated STA not found: " << aid);
    NS_TEST_EXPECT_MSG_EQ(addrByAid.value(), staAddress, "Retrieved wrong address by AID");

    Simulator::Schedule(TimeStep(1), [this, apMac, aid] {
        NS_TEST_EXPECT_MSG_EQ(apMac->GetMldOrLinkAddressByAid(aid).has_value(),
                              false,
                              "AID of disassociated STA still found: " << aid);
    });
}
...
WifiDisassocAndReassocTest::CheckAfterDisassoc(Ptr<WifiMac> local)
{
    ...
    for (const auto& [staLinkId, apLinkId] : m_params.firstSetupLinks)
    {
        ...
        for (const auto& [aci, ac] : wifiAcList)
        {
            for (const auto& queueId : std::initializer_list<WifiContainerQueueId>{
                     MakeWifiUnicastQueueId(WIFI_QOSDATA_QUEUE, addr, ac.GetLowTid()),
                     MakeWifiUnicastQueueId(WIFI_QOSDATA_QUEUE, addr, ac.GetHighTid()),
                     MakeWifiUnicastQueueId(WIFI_CTL_QUEUE, addr),
                     MakeWifiUnicastQueueId(WIFI_CTL_QUEUE, linkAddr)})
            {
                if (auto mask =
    ...
    if (localIsSender)
    {
        ...
        NS_TEST_EXPECT_MSG_EQ(
            acBe->GetNPackets(MakeWifiUnicastQueueId(WIFI_CTL_QUEUE, remoteBaLinkAddr)),
...
WifiDisassocAndReassocTest::InsertEvents()
{
    ...
    m_events.emplace_back(
        ...
        [=, this](Ptr<const WifiPsdu> psdu, const WifiTxVector& txVector, linkId_t linkId) {
            ...
            {
                const auto queueId =
                    MakeWifiUnicastQueueId(WIFI_CTL_QUEUE,
