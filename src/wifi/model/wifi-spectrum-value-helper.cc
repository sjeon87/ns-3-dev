                                                          ...
                                                          dBr_u lowestPoint)
{
    ...
    const auto start1 = (nGuardBands / 2) + tonePlan->skippedSubbands;
    const auto stop1 = start1 + (tonePlan->usedTones / 2) - 1;
    const auto start2 = stop1 + tonePlan->numDc + 1;
    const auto stop2 = start2 + (tonePlan->usedTones / 2) - 1;
    ...
    dBr_u lowestPoint)
{
    ...
    NS_ASSERT_MSG(c->GetSpectrumModel()->GetNumBands() ==
                      (nAllocatedBands + nGuardBands + nUnallocatedBands + 1),
                  "Unexpected number of bands " << c->GetSpectrumModel()->GetNumBands());
    const auto num20MhzBands = Count20MHzSubchannels(channelWidth);
    ...
    const std::vector<bool>& puncturedSubchannels)
{
    ...
    const auto num80MhzSegments =
        static_cast<std::size_t>((channelWidth >= 160) ? (channelWidth / 80) : 1);
