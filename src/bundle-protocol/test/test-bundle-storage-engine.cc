...
BundleStorageEngineTestCase::DoRun()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(storageEngine->GetTotalSize(), b1Size * 2, "BundleEngine total size not updated");

    uint32_t handle1 = storageEngine->StoreBundle(b1);
    NS_TEST_ASSERT_MSG_EQ(storageEngine->HasBundle(handle1), true, "Bundle 1 not inserted correctly");
    NS_TEST_ASSERT_MSG_EQ(storageEngine->GetCurrentSize(), b1Size, "BundleEngine size not incremented");

    Ptr<Bundle> retrieved = storageEngine->RetrieveBundle(handle1);
    NS_TEST_ASSERT_MSG_EQ(retrieved->GetTotalSize(), b1->GetTotalSize(), "Retrieved bundle does not match stored bundle");

    NS_TEST_ASSERT_MSG_EQ(storageEngine->RetrieveBundle(999), nullptr, "RetrieveBundle should return null for invalid handle");
    ...
    NS_TEST_ASSERT_MSG_EQ(storageEngine->GetCurrentSize(), b1Size * 2, "BundleEngine size not updated for second bundle");
    ...
    NS_TEST_ASSERT_MSG_EQ(storageEngine->GetCurrentSize(), b1Size * 2, "BundleEngine size should not change on rejection");
    ...
    std::vector<uint32_t> emptyHandles = storageEngine->GetHandlesForDestination("dtn:unknown-node");
    ...
    NS_TEST_ASSERT_MSG_EQ(storageEngine->GetCurrentSize(), b1Size, "BundleEngine size not decremented correctly");

    
    NS_TEST_ASSERT_MSG_EQ(storageEngine->DeleteBundle(999), 1, "DeleteBundle should return 1 for invalid handle");

    
    storageEngine->DeleteBundle(handle2);
    NS_TEST_ASSERT_MSG_EQ(storageEngine->GetCurrentSize(), 0, "BundleEngine size should be 0 after clearing");
}
...
static BundleStorageEngineTestSuite g_bundleStorageEngineTestSuite; //!< Static variable for test initialization