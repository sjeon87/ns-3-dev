...
{
  public:
    BundleProtocolTestCase(uint32_t sentBundleSize,
                           uint32_t bundleSize,
...
BundleProtocolTestCase::BundleProtocolTestCase(uint32_t sentBundleSize,
                                               uint32_t bundleSize,
                                               uint32_t segmentSize,
                                               std::string claType)
    : TestCase("Test that all the bundles generated by a sender bundle node are correctly received "
               "by a receiver bundle node"),
...
BundleProtocolTestCase::DoRun(void)
{
    ...
    Simulator::Schedule(Seconds(0.8),
                        &BundleProtocolTestCase::Receive,
                        this,
                        bpReceivers.Get(0),
                        eidRecv);
    ...
    NS_TEST_EXPECT_MSG_EQ(m_receivedBundleSize,
                          m_sentBundleSize,
                          "All bundles are received at the receiver");
    NS_TEST_EXPECT_MSG_EQ(m_receivedBundleNumber,
                          num,
                          "Correct number of bundles are received at the receiver");
}

void
BundleProtocolTestCase::Send(Ptr<BundleProtocol> sender,
                             uint32_t size,
                             BpEndpointId src,
                             BpEndpointId dst)
{
