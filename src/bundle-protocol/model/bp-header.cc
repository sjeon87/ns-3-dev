...
{
    static TypeId tid = TypeId("ns3::BpHeader").SetParent<Header>().AddConstructor<BpHeader>();
...
BpHeader::Serialize(Buffer::Iterator start) const
{
    ...
    for (std::vector<uint8_t>::iterator it = processingFlags.begin(); it != processingFlags.end();
         ++it)
    {
        i.WriteU8(*it);
    }
    ...
    for (std::vector<uint8_t>::iterator it = headerLengthSDNV.begin(); it != headerLengthSDNV.end();
         ++it)
    {
        i.WriteU8(*it);
    }
    ...
    // Header body
    for (std::vector<uint8_t>::iterator it = headerBody.begin(); it != headerBody.end(); ++it)
    {
        i.WriteU8(*it);
    }
}
...
BpHeader::Deserialize(Buffer::Iterator start)
{
    ...
    std::vector<char> str(m_dictLength);
    for (uint32_t k = 0; k < m_dictLength; k++)
    {
        str[k] = i.ReadU8();
    }
    m_dictionary = std::string(str.data(), m_dictLength);
    if (m_processingFlags & BUNDLE_IS_FRAGMENT)
    {
...
BpHeader::GetReportEid() const
{
    NS_LOG_FUNCTION(this);
    std::string scheme =
        m_dictionary.substr(m_reportSchemeOffset.offset, m_reportSchemeOffset.length);
