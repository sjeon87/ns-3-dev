...
class BundleProtocol : public Object
{
  ...
  private:
    ...
    std::map<BpEndpointId, std::queue<Ptr<Packet>>>
        BpSendBundleStore; /// persistant storage of sent bundles: map (source endpoint id, bundle
                           /// packet queue )
    std::map<BpEndpointId, std::queue<Ptr<Packet>>>
        BpRecvBundleStore; /// persistant storage of received bundles: map (destination endpoint id,
                           /// bundle packet queue )
    std::map<BpEndpointId, BpRegisterInfo>
        BpRegistration; /// persistant storage of registrations: map (local endpoint id,
                        /// registration information)

    Ptr<Packet> m_bpRxBufferPacket; /// a buffer for all packets received from the CLA; bundles are
                                    /// retreived from this buffer
