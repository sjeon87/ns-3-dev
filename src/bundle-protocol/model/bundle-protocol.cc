...
BundleProtocol::GetTypeId(void)
{
    static TypeId tid = TypeId("ns3::BundleProtocol")
                            ...
                            .AddAttribute("BundleSize",
                                          "Max size of a bundle in bytes",
                                          UintegerValue(512),
                            ...
                            .AddAttribute("StartTime",
                                          "Time at which the bundle protocol will start",
                                          TimeValue(Seconds(0.0)),
                                          MakeTimeAccessor(&BundleProtocol::m_startTime),
                                          MakeTimeChecker())
                            .AddAttribute("StopTime",
                                          "Time at which the bundle protocol will stop",
...
BundleProtocol::Open(Ptr<Node> node)
{
    ...
    {
        NS_FATAL_ERROR("BundleProtocol::Open (): unkonw tranport layer protocol type! "
                       << m_l4Type);
    }
}
...
BundleProtocol::Send(Ptr<Packet> p, const BpEndpointId& src, const BpEndpointId& dst)
{
    ...
    while (total > 0)
    {
        ...
        NS_LOG_DEBUG("Send bundle:" << " seq " << bph.GetSequenceNumber().GetValue() << " src eid "
                                    << bph.GetSourceEid().Uri() << " dst eid "
                                    << bph.GetDestinationEid().Uri() << " pkt size "
                                    << packet->GetSize());
...
BundleProtocol::RetreiveBundle()
{
    ...
    // continue to retreive a bundle from buffer until the buffer size is smaller than a bundle or a
    // bundle header
    if (m_bpRxBufferPacket->GetSize() >
        (bpHeader.GetSerializedSize() + bppHeader.GetSerializedSize()))
    {
        // since BpHeader's length is a variable, we must also read data buffer size to guarantee
        // that the node receives a complete primary bundle header and bundle payload header
        ...
        // if there is not data, we cannot call PeekHeader, since we cannot guarantee a complete
        // primary bundle header and bundle header is received
        if (buf.GetSize() == 0)
        {
            return;
        }
        ...
        uint32_t total = bpHeader.GetBlockLength() + bpHeader.GetSerializedSize() +
...
BundleProtocol::ProcessBundle(Ptr<Packet> bundle)
{
    ...
    NS_LOG_DEBUG("Recv bundle:" << " seq " << bpHeader.GetSequenceNumber().GetValue() << " src eid "
                                << bpHeader.GetSourceEid().Uri() << " dst eid "
                                << bpHeader.GetDestinationEid().Uri() << " packet size "
                                << bundle->GetSize());
...
BundleProtocol::StartBundleProtocol()
{
    ...
    {
        NS_FATAL_ERROR(
            this << " BundleProtocol::DoInitialize (): do not define convergence layer! ");
    }

    if (m_cla->GetRoutingProtocol() == nullptr)
    {
        NS_FATAL_ERROR(
            this << " BundleProtocol::DoInitialize (): do not define bundle routing protocol! ");
    }
