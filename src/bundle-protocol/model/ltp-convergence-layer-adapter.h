...
class LtpConvergenceLayerAdapter : public Object
{
  public:
    ...
    /*
     * @brief switches link status to "down" and notifies upper layer, if already down it does
     * nothing.
     */
  ...
  protected:
    ...
    Callback<void, Ptr<LtpConvergenceLayerAdapter>>
        m_linkUp; //!< Callback used to notify link ready status.
    Callback<void, Ptr<LtpConvergenceLayerAdapter>>
        m_linkDown; //!< Callback used to notify link down status.

    /* Segment transmission callbacks */

    Callback<void, SessionId, RedSegmentInfo>
        m_checkpointSent; //!< Notify transmission of a checkpoint segment
    Callback<void, SessionId, RedSegmentInfo>
        m_reportSent; //!< Callback used to notify that Report has been transmitted
