...
{
    static TypeId tid =
        TypeId("ns3::BpPayloadHeader").SetParent<Header>().AddConstructor<BpPayloadHeader>();
...
BpPayloadHeader::Serialize(Buffer::Iterator start) const
{
    ...
    for (std::vector<uint8_t>::iterator it = payloadLength.begin(); it != payloadLength.end(); ++it)
    {
        i.WriteU8(*it);
    }

    // Payload
    for (std::vector<uint8_t>::const_iterator it = m_payload.begin(); it != m_payload.end(); ++it)
    {
        i.WriteU8(*it);
    }
}
