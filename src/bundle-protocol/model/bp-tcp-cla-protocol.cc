...
BpTcpClaProtocol::EnableReceive(const BpEndpointId& local)
{
    ...
    // store the sending socket so that the convergence layer can dispatch the hundles to different
    // tcp connections
...
BpTcpClaProtocol::EnableSend(const BpEndpointId& src, const BpEndpointId& dst)
{
    ...
    InetSocketAddress defaultAddr("127.0.0.1", 0);
    if (addr.GetIpv4() == defaultAddr.GetIpv4() && addr.GetPort() == defaultAddr.GetPort())
    {
        NS_LOG_DEBUG(
            "BpTcpClaProtocol::EnableSend (): cannot find route for destination endpoint id "
            << dst.Uri());
        return -1;
    }
...
BpTcpClaProtocol::SetL4SocketCallbacks(Ptr<Socket> socket)
{
    ...
    socket->SetConnectCallback(MakeCallback(&BpTcpClaProtocol::ConnectionSucceeded, this),
                               MakeCallback(&BpTcpClaProtocol::ConnectionFailed, this));

    socket->SetCloseCallbacks(MakeCallback(&BpTcpClaProtocol::NormalClose, this),
                              MakeCallback(&BpTcpClaProtocol::ErrorClose, this));

    socket->SetAcceptCallback(MakeCallback(&BpTcpClaProtocol::ConnectionRequest, this),
                              MakeCallback(&BpTcpClaProtocol::NewConnectionCreated, this));

    socket->SetDataSentCallback(MakeCallback(&BpTcpClaProtocol::DataSent, this));

    socket->SetSendCallback(MakeCallback(&BpTcpClaProtocol::Sent, this));

    socket->SetRecvCallback(MakeCallback(&BpTcpClaProtocol::DataRecv, this));
}
