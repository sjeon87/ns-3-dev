      ...
      if (currNode == source && oif)
        {
          ...
          Ptr<NetDeviceState> oifDevState = oif->GetObject<NetDeviceState> ();
          bool oifLinkUp = oifDevState != nullptr ? (oifDevState->GetOperationalState () == NetDeviceState::IF_OPER_UP) : (oif->IsLinkUp ());
          ...
          for (uint32_t i = 0; i < (currNode->GetNDevices ()); i++)
            {
              ...
              Ptr<NetDeviceState> localDevState = localNetDevice->GetObject<NetDeviceState> ();
              bool localLinkUp = localDevState != nullptr ? (localDevState->GetOperationalState () == NetDeviceState::IF_OPER_UP) : (localNetDevice->IsLinkUp ());
