...
static constexpr std::array highSpeedImportantValues{std::to_array<HighSpeedImportantValues>(
...
/**
 * Construct the TestCase name from the @c cwnd parameter.
 * @param increment TcpHighSpeedIncrementTest if @c true, TcpHighSpeedDecrementTest if @c false.
 * @param cwnd The congestion window being tested.
 * @returns The TestCase name.
 */
static std::string
Name(bool increment, int cwnd)
{
    std::ostringstream oss;
    oss << "Highspeed " << (increment ? "increment" : "decrement") << " test on cWnd " << cwnd;
    return oss.str();
}
...
class TcpHighSpeedTestSuite : public TestSuite
{
  public:
    ...
    {
        for (const auto& hsiv : highSpeedImportantValues)
        {
            AddTestCase(new TcpHighSpeedIncrementTest(hsiv.cwnd, 1, Name(true, hsiv.cwnd)),
                        TestCase::Duration::QUICK);
            AddTestCase(
                new TcpHighSpeedIncrementTest(hsiv.cwnd * 536, 536, Name(true, hsiv.cwnd * 536)),
                TestCase::Duration::QUICK);
            AddTestCase(
                new TcpHighSpeedIncrementTest(hsiv.cwnd * 1446, 1446, Name(true, hsiv.cwnd * 1446)),
                TestCase::Duration::QUICK);
            AddTestCase(new TcpHighSpeedDecrementTest(hsiv.cwnd, 1, Name(false, hsiv.cwnd)),
                        TestCase::Duration::QUICK);
            AddTestCase(
                new TcpHighSpeedDecrementTest(hsiv.cwnd * 536, 536, Name(false, hsiv.cwnd * 536)),
                TestCase::Duration::QUICK);
            AddTestCase(new TcpHighSpeedDecrementTest(hsiv.cwnd * 1446,
                                                      1446,
                                                      Name(false, hsiv.cwnd * 1446)),
                        TestCase::Duration::QUICK);
        }
    }
};
