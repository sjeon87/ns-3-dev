...
ArpCache::LookupInverse(Address to)
{
    ...
    if (!m_reverseIndexValid)
    {
        m_reverseIndex.clear();
        for (auto& [ip, entry] : m_arpCache)
        {
            m_reverseIndex[entry->GetMacAddress()].push_back(entry);
        }
        m_reverseIndexValid = true;
    }

    std::list<ArpCache::Entry*> entryList;
    auto it = m_reverseIndex.find(to);
    if (it != m_reverseIndex.end())
    {
        entryList.assign(it->second.begin(), it->second.end());
    }
    return entryList;
}
...
ArpCache::Entry::MarkAlive(Address macAddress)
{
    ...
    m_arp->m_reverseIndexValid = false;
    m_state = ALIVE;
    ClearRetries();
    UpdateSeen();
}
