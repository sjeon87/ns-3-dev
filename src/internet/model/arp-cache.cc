...
{
    NS_LOG_FUNCTION(this);
    return (m_state == REACHABLE || m_state == STALE || m_state == DELAY || m_state == PROBE);
}
...
{
    NS_LOG_FUNCTION(this);
    return (m_state == INCOMPLETE || m_state == PROBE);
}

bool
ArpCache::Entry::IsStale()
{
    NS_LOG_FUNCTION(this);
...
void
ArpCache::Entry::MarkDelay()
{
    NS_LOG_FUNCTION(this);
    m_state = DELAY;
    m_expiry = Simulator::Now() + Seconds(5);
    UpdateSeen();
}
...
ArpCache::Entry::MarkDead()
{
    NS_LOG_FUNCTION(this);
    m_state = FAILED;
    m_expiry = Simulator::Now() + m_arp->GetDeadTimeout();
    ClearRetries();
    UpdateSeen();
}
...
ArpCache::Entry::MarkAlive(Address macAddress)
{
    ...
    m_state = REACHABLE;
    m_expiry = Simulator::Now() + m_arp->GetAliveTimeout();
    ClearRetries();
    UpdateSeen();
}
...
ArpCache::Entry::UpdateWaitReply(Ipv4PayloadHeaderPair waiting)
{
    ...
    NS_ASSERT(m_state == INCOMPLETE || m_state == DELAY || m_state == PROBE);
...
{
    NS_LOG_FUNCTION(this << waiting.first);
    NS_ASSERT(m_state != PERMANENT && m_state != STATIC_AUTOGENERATED);
...
ArpCache::Entry::GetTimeout() const
{
    ...
    switch (m_state)
    {
    case ArpCache::Entry::INCOMPLETE:
    case ArpCache::Entry::PROBE:
    case ArpCache::Entry::DELAY:
        return m_arp->GetWaitReplyTimeout();
    case ArpCache::Entry::FAILED:
        return m_arp->GetDeadTimeout();
    case ArpCache::Entry::REACHABLE:
    case ArpCache::Entry::STALE:
...
{
    NS_LOG_FUNCTION(this);
    if (m_state == PERMANENT || m_state == STATIC_AUTOGENERATED)
    {
        return false;
    }

    if (Simulator::Now() >= m_expiry)
    {
        // If it was REACHABLE but time is up, it becomes STALE
        if (m_state == REACHABLE)
        {
            const_cast<ArpCache::Entry*>(this)->m_state = STALE;
            return false; // STALE entries aren't "expired" yet, they're just old
        }
        return true; // Other states (like FAILED or INCOMPLETE) are truly expired
    }
    return false;
}
