...
struct Hz_t
{
    ...
    explicit Hz_t(const std::string& str){
        ...
        UNITS_ASRT(res.has_value(), GetParseErrMsg(str, "Hz"));
        val = res.value().val;
    }
...
struct kHz_t : Hz_t
{
    ...
    {
        auto res = from_str(str);
        UNITS_ASRT(res.has_value(), GetParseErrMsg(str, "kHz"));
        val = res.value().val;
    }
...
struct MHz_t : Hz_t
{
    ...
    {
        auto res = from_str(str);
        UNITS_ASRT(res.has_value(), GetParseErrMsg(str, "MHz"));
        val = res.value().val;
    }
...
struct GHz_t : Hz_t
{
    ...
    {
        auto res = from_str(str);
        UNITS_ASRT(res.has_value(), GetParseErrMsg(str, "GHz"));
        val = res.value().val;
    }
...
struct THz_t : Hz_t
{
    ...
    {
        auto res = from_str(str);
        UNITS_ASRT(res.has_value(), GetParseErrMsg(str, "THz"));
        val = res.value().val;
    }
...
// Vector support
using HzVec_t = SiUnitsVec<Hz_t>;
using kHzVec_t = SiUnitsVec<kHz_t>;
using MHzVec_t = SiUnitsVec<MHz_t>;
using GHzVec_t = SiUnitsVec<GHz_t>;
using THzVec_t = SiUnitsVec<THz_t>;
