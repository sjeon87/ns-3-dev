...
struct dB_t
{
    ...
    explicit dB_t(const std::string& str)
    {
        ...
        UNITS_ASRT(res.has_value(), GetParseErrMsg(str, "dB"));
        val = res.value().val;
    }
...
struct dBm_t
{
    ...
    {
        auto res = from_str(str);
        UNITS_ASRT(res.has_value(), GetParseErrMsg(str, "dBm"));
        val = res.value().val;
    }
...
struct mWatt_t
{
    ...
    {
        auto res = from_str(str);
        UNITS_ASRT(res.has_value(), GetParseErrMsg(str, "mWatt"));
        val = res.value().val;
    }
    ...
    {
        return mWatt_t{ToLinearScale(input.val)};
    }
...
struct Watt_t
{
    ...
    {
        auto res = from_str(str);
        UNITS_ASRT(res.has_value(), GetParseErrMsg(str, "Watt"));
        val = res.value().val;
    }
    ...
    {
        return Watt_t{ToLinearScale(input.val - 30.0)}; // -30 == 10log10(1/1000)
    }
    ...
    {
        return Watt_t{(input.val / ONE_KILO)};
    }
...
struct dBm_per_Hz_t // NOLINT(readability-identifier-naming)
{
    ...
    {
        auto res = from_str(str);
        UNITS_ASRT(res.has_value(), GetParseErrMsg(str, "dBm/Hz"));
        val = res.value().val;
    }
...
struct dBm_per_MHz_t // NOLINT(readability-identifier-naming)
{
    ...
    {
        auto res = from_str(str);
        UNITS_ASRT(res.has_value(), GetParseErrMsg(str, "dBm/MHz"));
        val = res.value().val;
    }
...
// Vector support
using dBVec_t = SiUnitsVec<dB_t>;
using dBrVec_t = SiUnitsVec<dBr_t>;
using dBmVec_t = SiUnitsVec<dBm_t>;
using mWattVec_t = SiUnitsVec<mWatt_t>;
using WattVec_t = SiUnitsVec<Watt_t>;
